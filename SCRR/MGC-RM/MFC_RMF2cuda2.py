...
from SCRR.utils.model import GAT
...
x_s_tensor = torch.tensor(fea_ori['arr_0'], dtype=torch.float32).to(device)
x_t_tensor = torch.tensor(perturbed_f, dtype=torch.float32).to(device)
#print(fea_ori['arr_0'].shape)
#print(perturbed_f.shape)
edge_index_s_tensor = torch.tensor(edge_index_s, dtype=torch.int64).to(device)
edge_index_t_tensor = torch.tensor(edge_index_t, dtype=torch.int64).to(device)
label_tensor = torch.tensor(perturbed_l, dtype=torch.int64).to(device)
...
print(args)
model = MFC_RMF(args.input_dim, args.hidden_dim, args.output_dim, args).to(device)
# print(summary(model))
...
z_targe = torch.Tensor([similarity]).to(device)  # 创建一个新对象（如张量）时，提供的数据应该是一个序列（sequence）
...
for i in range(perturbed_a.shape[0]):
    ...
    x_t_tensor2 = torch.tensor(perturbed_f2, dtype=torch.float32).to(device)
    # print(fea_ori['arr_0'].shape)
    # print(perturbed_f.shape)
    # edge_index_s_tensor = torch.tensor(edge_index_s, dtype=torch.int64)
    edge_index_t_tensor2 = torch.tensor(edge_index_t2, dtype=torch.int64).to(device)
    ...
    z_targe2 = torch.Tensor([similarity2]).to(device)  # 创建一个新对象（如张量）时，提供的数据应该是一个序列（sequence）
...
# plt.text(round(args.max_epoch / 10 * 9), loss_history2[(args.max_epoch - 1)],
        ...
        # horizontalalignment='left')
plt.savefig('./prediction_loss/GraphPair_' + str(j) + '_n' + str(node) + '_d' + str(data_num) + '_Similarity_Loss_epoch_' + str(
